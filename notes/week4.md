...
- $\alpha$ &larr; learning rate
- $\frac{\partial}{\partial \theta_j} J(\theta_0, \theta_1)$ &larr; derivative
- $\underset{\theta_1}{\text{min }} J(\theta_1) \quad \theta_1 \text{ is a real number}$
...
To fix this, as we approach a local minimum, gradient descent must be programmed to automatically take smaller steps. So, no need to decrease $\alpha$ over time.
